...
<div style="display:flex; flex-direction:column; align-items:center; justify-content:center; width:100%; height:100%;">
        <h2>Vnesi gi rezultatite od laboratoriskiot ispit</h2>
            <div style="display:flex;flex-direction:row;width:30%;margin-bottom:10px;margin-top:50px;">
                ...
                <select name="pero" id="maticen" onchange="smeni_pacient(this)" class="form-control">
                    ...
                    @foreach($maticni as $maticen)
                        <option value="{{$maticen->id}}" placeholder="Maticen doktor">{{$maticen->first_name}}</option>
                    @endforeach
                </select>
            </div>
                     ...
                     <div style="display:flex;flex-direction:row;justify-content:center;width:30%;">
                        ...
                        <select name="user_id" id="pacient" class="form-control">
                            ...
                            @foreach($pacienti as $pacient)
                                <option id="{{$pacient->maticen_id}}" value="{{$pacient->id}}" placeholder="Pacient">{{$pacient->first_name}}</option>
                             @endforeach
                        </select>
                     </div>
            </div>
            @foreach($items as $item)
                ...
                <div style="display:flex;flex-direction:column;width:30%">
                        ...
                        <input type="hidden" name="name{{$item->id}}" value="{{$item->name}}" class="form-control" placeholder="Pacient" >
                    <div style="display:flex;flex-direction:row;width:100%;margin-bottom:10px;">
                        <label style="width:25%;">{{$item->name}}</label>
                        <input type="text" name="value{{$item->id}}" class="form-control" placeholder="Vrednost za {{$item->name}} ">
                    </div>
                        <input type="hidden" name="item_id{{$item->id}}" value="{{$item->id}}" class="form-control" placeholder="Pacient">
                </div>
            @endforeach

            <button type="submit" class="btn btn-primary" style="background-color:#00a4a2;border:none;">Potvrdi</button>
        </form>
</div>
    @endsection